...
public class ImageProc{
	 ...
	 */
	public ImageProc(byte[] src, int width, int height) {
		if (src.length != width*height*3) {
			throw new IllegalArgumentException(
			"Length "+src.length+" does not match width and height.");
		}
		pixels = src;
		this.width = width;
		this.height = height;
	 ...
	 * @param height
	 */
	public ImageProc(int width, int height) {
	 ...
	 **/
	public ImageProc(ImageProc image) {
